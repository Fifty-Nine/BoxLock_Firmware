...
#define CONF_STORAGE_MEMORY_START 0x3f000
#endif
 ...
 */
#ifdef FLASH_SIZE
#if (CONF_STORAGE_MEMORY_START + (CONF_MAX_ITEM_NUMBER * CONF_SECTOR_SIZE)) > FLASH_SIZE
#error Invalidate storage configuration, make sure the configuration item suit for device flash size.
#endif
#endif

#ifdef IFLASH_SIZE
#if (CONF_STORAGE_MEMORY_START + (CONF_MAX_ITEM_NUMBER * CONF_SECTOR_SIZE)) > IFLASH_SIZE
#error Invalidate storage configuration, make sure the configuration item suit for device flash size.
#endif
#endif
