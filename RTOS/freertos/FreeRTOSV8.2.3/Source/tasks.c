                                                                  ...
                                                                  strings and single characters only. */
{
	...
	if ((pxTaskBuffer != NULL) && (puxStackBuffer != NULL)) {
		...
		function - use them. */
		pxNewTCB = (TCB_t *)pxTaskBuffer; /*lint !e740 Unusual cast is ok as the structures are designed to have the
...
static void prvInitialiseNewTask(
    ...
    TCB_t *pxNewTCB) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
...
#if (portSTACK_GROWTH < 0)
	{
		...
		pxTopOfStack
		    = (StackType_t *)(((portPOINTER_SIZE_TYPE)pxTopOfStack)
		                      & (~(
		                            (portPOINTER_SIZE_TYPE)portBYTE_ALIGNMENT_MASK))); /*lint !e923 MISRA exception.
		                                                                                  Avoiding casts between
		                                                                                  pointers and integers is not
...
/* Add the idle task at the lowest priority. */
#if (configSUPPORT_STATIC_ALLOCATION == 1)
	{
		...
		xReturn = xTaskCreateStatic(prvIdleTask,
		                            "IDLE",
		                            usIdleTaskStackSize,
		                            (void *)NULL,
...
TaskHandle_t xTaskGetTaskHandle(const char *pcNameToQuery) /*lint !e971 Unqualified char types are allowed for strings
                                                              and single characters only. */
{
